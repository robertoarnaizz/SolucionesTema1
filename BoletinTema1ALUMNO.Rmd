...
rownames(df_animals2)
colnames(df_animals2)
...
#Apartado d
commonAnimals <- intersect(rownames(df_animals2), rownames(df_mammals)) #Uso rownames, porque solo quiero comparar los nombres de los animales.
commonAnimals
...
```{r}
#Animales en mammals pero no en Animals2
noncommonAnimals <- setdiff(rownames(df_mammals), rownames(df_animals2))
noncommonAnimals

#Animales en Animals2 pero no en mammals
noncommonAnimals1 <- setdiff(rownames(df_animals2), rownames(df_mammals))
noncommonAnimals1
```
...
#Mamals
qqPlot(log(df_mammals$brain))

#Animals2
qqPlot(log(df_Animals2$brain))

#Ambas distribuciones presentan el mismo comprtamiento, como las muestras estan dentro de las lineas podemos afirmar que ambas siguen una distribución Gaussiana.
...
#Apartado c
nrow(libReturn$results) #El número de librerias es igual al numero de filas de la matriz que contiene las librerias instaladas.
...
#Conjunto de datos cabbage

#Apartado a
library(MASS) #Cargamos la librería necesaria.
nrow(cabbages)
ncol(MASS::cabbages) #Otra forma es acceder asi.
str(cabbages)
sum(is.na(cabbages)) #Número de valores perdidos en todo el data frame
...
#Apartado a
library(car) #Cargamos la librería necesaria.
nrow(Chile)
ncol(Chile)
str(Chile)
sum(is.na(Chile))
...
#Como podemos ver en los gráficos todos los puntos estan dentro de las lineas y por tanto podemos asumir que ambas variables siguen una distribución Gaussiana
...
levels(cabbages$Cult) #Obtenemos que variedades de cultivo hay

#Mostramos los histogramas para cada tipo de cultivo
...
#No existe una relación lineal entre los niveles de vitamina c y el peso de la calabaza.
...
#Diagrama de dispersión etiquetado segun tipo de cultivo
plot(cabbages$HeadWt, cabbages$VitC, pch = c(6, 16)[cabbages$Cult])
legend(x = "topright", legend = c("Cult = c39", "Cult = c52"), pch = c(6, 16))

#Planteo 2 modelos lineales, uno para los datos de cada factor
Modelo1 <- lm(VitC ~ HeadWt, data = cabbages, subset = which(Cult == "c52"))
Modelo2 <- lm(VitC ~ HeadWt, data = cabbages, subset = which(Cult == "c39"))
Modelo3 <- lm(VitC ~ HeadWt, data = cabbages)

#Respresentamos las resctas de cada modelo lineal
abline(Modelo1)
